...
class AddKTrigramLM(BaseLM):
    ...
    def __init__(self, tokens):
        ...
        for key, value in self.counts.items():
            self.context_totals[key] = sum(value.values())
    ...
    def next_word_proba(self, word, seq):
        ...
        self.C_abc = 0.0
        if self.counts.get(context, 0.0) == 0.0:
            self.C_abc = 0.0
        else:
            self.C_abc = self.counts.get(context, 0.0).get(word, 0.0)
                
        self.C_ab = self.context_totals.get(context, 0.0)
        
        return (self.C_abc +self.k)/(self.C_ab +(self.k*self.V))

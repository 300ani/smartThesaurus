...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        with tf.variable_scope("Recurrent_Layer"):
            ...
            self.outputs_, self.final_h_ = tf.nn.dynamic_rnn(self.cell_, 
                                                             self.x_, 
        ...
        with tf.variable_scope("Output_Layer"):
            self.W_out_ = tf.get_variable(name="W_out", shape=(self.H, self.V), 
                                          initializer=tf.random_uniform_initializer(minval=-1.0, maxval=1.0))
            self.b_out_ = tf.get_variable(name="b_out", shape=(self.V), 
                                          initializer=tf.zeros_initializer())
            self.logits_ = matmul3d(self.outputs_, self.W_out_) + self.b_out_

        # Loss computation (true loss, for prediction)
        with tf.variable_scope("Softmax"):
            self_loss_ = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits_, labels=self.target_y_)
            self.loss_ = tf.reduce_mean(self_loss_)
    ...
    def BuildTrainGraph(self):
        ...
        with tf.variable_scope("Training"):
            ...
            train_op_ = tf.train.AdamOptimizer(learning_rate=self.learning_rate_)
    ...
    def BuildSamplerGraph(self):
        ...
        with tf.variable_scope("Build_Sample"):
            self.sample_ = tf.multinomial(tf.reshape(self.logits_, [-1, self.V]), 1)
            self.pred_samples_ = tf.reshape(self.sample_, [self.batch_size_, self.max_time_, 1])
